...
# ---------------- Architecture personnalisée ResNet50 avec SE ----------------
class ResNet50WithSE(nn.Module):
    def __init__(self, num_classes=9, hidden_size=512, dropout=0.5, reduction=16):
        ...
        # Extraire les couches de features sans le pooling final
        ...
        # Ajout du bloc SE sur les features (la sortie de layer4 a 2048 canaux)
        self.se = SEBlock(2048, reduction=reduction)
        # Pooling global identique à celui de ResNet
    ...
    def forward(self, x):
        ...
        x = self.se(x)            # Application de l'attention
...
class TestFolderDataset(Dataset):
    def __init__(self, img_dir, transform=None, extension=".jpg"):
        ...
        self.img_files.sort()  # pour garantir la reproductibilité
...
def train_model(model, criterion, optimizer, scheduler, dataloaders, num_epochs=30, device='cuda', early_stopping_patience=10, use_mixup=False):
    ...
    for epoch in range(num_epochs):
        ...
        # Mise à jour du scheduler (ici, CosineAnnealingLR)
        scheduler.step()
...
def main_train(args):
    ...
    # Augmentations spatiales pour l'entraînement
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(df, df['label'])):
        ...
        # Utilisation de notre modèle ResNet50 avec SE et deux couches denses
        model = ResNet50WithSE(num_classes=9, hidden_size=512, dropout=0.5, reduction=16)
        model = model.to(device)

        # Calcul des poids pour la Focal Loss selon la distribution des classes
        ...
        # Scheduler : CosineAnnealingLR sur le nombre total d'époques
...
def main_test(args):
    ...
    output_csv = "predictions_test_ensemble4.csv"
