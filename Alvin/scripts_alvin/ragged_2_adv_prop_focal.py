...
focal_weights = np.zeros((9))
for key in generator.class_freqs :
    w = generator.class_freqs[key] 
    focal_weights[key] = w
focal_weights[8] = focal_weights[8] * 5
focal_weights = 1 - (focal_weights / np.sum(focal_weights))
print(focal_weights)
...
def create_mlp() :
    ...
    l1 = layers.LeakyReLU()(l1)
    l1 = layers.Dropout(0.4)(l1)
    lout = layers.Dense(9, activation='linear')(l1)   # retourne logits
...
for epoch in range(50) :
    ...
    for batch in generator :
        ...
        with tf.GradientTape() as tape :
            ...
            logits = mlp(features, training=True)
            #probs = tf.exp(logits) / tf.reduce_sum(tf.exp(logits), axis=1, keepdims=True)
            probs = tf.nn.softmax(logits)
            ce = -batch["train_labels"] * tf.math.log(probs)
            wce = focal_weights * ce
            gamma = 2.0  # Ajustable
            focal_weight = tf.pow(1 - probs, gamma)
            loss_classif = tf.reduce_mean(focal_weight * wce)
