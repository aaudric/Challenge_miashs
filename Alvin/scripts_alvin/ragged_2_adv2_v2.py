...
class RaggedGenerator(keras.utils.Sequence) :
    ...
    def _load_from_csv(self, data_dir, csv_path):
        ...
        for _, row in df.iterrows():
            label = np.zeros((9))
            label[int(row["label1"])] += 1
            label[int(row["label2"])] += 1
            label[int(row["label3"])] += 1
            label[int(row["label4"])] += 1
            label /= np.sum(label)
    ...
    def __getitem__(self, idx):
        ...
        batch_img = tf.image.random_contrast(batch_img, lower=0.7, upper=1.3)
        ...
        test_img = tf.image.random_contrast(test_img, lower=0.7, upper=1.3)
        ...
        return {
            "train_img": tf.cast(batch_img, dtype=tf.float16),
            #"train_ratio": batch_ratios,
            "train_labels": batch_labels,
            "test_img": tf.cast(test_img, dtype=tf.float16),
...
backbone = keras.applications.ResNet50(input_shape=(244, 244, 3), weights="imagenet", include_top=False, pooling='avg')
mlp = create_mlp()
...
#backbone.load_weights("/home/barrage/grp3/models/alvin/backbone_600_adv_50_t01.weights.h5")
#mlp.load_weights("/home/barrage/grp3/models/alvin/mlp_600_adv_50_t01.weights.h5")
...
"""for epoch in range(10) :
    ...
    for batch in generator :
        ...
        with tf.GradientTape(persistent=True) as tape :
            features1 = backbone(batch["train_img"], training=False)
            features2 = backbone(batch["test_img"], training=False)
    ...
    print("EPOCH ", epoch, "END   ADV LOSS :", adv_loss.result(), adv_accuracy.result() )
        


adv.save_weights("/home/barrage/grp3/models/alvin/adv_mlp.weights.h5")
"""
#toto()
...
for epoch in range(80) :
    ...
    if (epoch+1) % 10 == 0 :
        backbone.save_weights("/home/barrage/grp3/models/alvin/backbone_"+str(epoch)+"_last_noadv.weights.h5")
        mlp.save_weights("/home/barrage/grp3/models/alvin/mlp_"+str(epoch)+"_last.weights.h5")
        print("JE SAVE")
...
backbone.save_weights("/home/barrage/grp3/models/alvin/backbone_last_noadv.weights.h5")
mlp.save_weights("/home/barrage/grp3/models/alvin/mlp_last_noadv.weights.h5")





backbone.load_weights("/home/barrage/grp3/models/alvin/backbone_last_noadv.weights.h5")
mlp.load_weights("/home/barrage/grp3/models/alvin/mlp_last_noadv.weights.h5")
